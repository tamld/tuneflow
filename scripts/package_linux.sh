...
if command -v sha256sum >/dev/null 2>&1; then
    sha256sum "TuneFlow-${VERSION}-linux-${ARCH_TAG}.tar.gz" > "SHA256SUMS-linux-${ARCH_TAG}.txt"
elif command -v shasum >/dev/null 2>&1; then
    shasum -a 256 "TuneFlow-${VERSION}-linux-${ARCH_TAG}.tar.gz" > "SHA256SUMS-linux-${ARCH_TAG}.txt"
fi

echo "🎉 Linux packaging complete!"
cat "SHA256SUMS-linux-${ARCH_TAG}.txt"
