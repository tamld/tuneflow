...
3. Endpoint `/api/download/:id/file` trên server gắn cờ tiêu đề:
   ...
   ```
4. Trình duyệt của Ba Mẹ nhận diện luồng binary và tự động kích hoạt tiến trình tải về thư mục `Downloads` của máy tính cá nhân.

## Hậu Quả (Consequences)
- **Tích cực**: Ba Mẹ chỉ cần bấm "Tải Về", sau khi nhạc chuyển đổi xong là tệp tự động rơi vào máy tính cá nhân. Xóa bỏ hoàn toàn khoảng cách giữa Server nội bộ và Client.
