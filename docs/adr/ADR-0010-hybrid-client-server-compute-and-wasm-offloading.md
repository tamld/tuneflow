...
2. **Kiến trúc Hybrid Client-Server Stream Pipelining**:
   ...
   - **Tùy chọn Zero-Disk Retention**: Máy chủ có thể stream trực tiếp từ YouTube CDN về thẳng trình duyệt client mà không cần lưu lại byte nào vào ổ cứng của Container Host, giải phóng 100% rủi ro đầy bộ nhớ đệm.
3. **Phân tải tính toán cho Client (Client Compute via WebAssembly)**:
   ...
   - Giảm tải 100% năng lượng xử lý hậu kỳ cho máy chủ.

## Hệ quả
- Giải đáp triệt để nguyên lý kỹ thuật: Server giải mã & pipe luồng; Client nhận stream và xử lý hiển thị/lưu trữ/hậu kỳ.
- Container Host chỉ tốn băng thông trung chuyển mạng LAN, CPU gần như nhàn rỗi.
