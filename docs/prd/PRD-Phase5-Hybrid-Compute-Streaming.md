...
Tối ưu hóa triệt để tài nguyên máy chủ Proxmox bằng cách phân định ranh giới tính toán rõ ràng giữa Server và Client: Server chịu trách nhiệm vượt rào cản CORS và giải mã cipher của YouTube, còn Client nhận luồng stream trực tiếp và xử lý các tác vụ hậu kỳ (cắt đoạn, chuẩn hóa âm thanh, gắn thẻ tag) thông qua WebAssembly (WASM).
...
- Nếu mọi tác vụ mux âm thanh phức tạp đều dồn lên CPU máy chủ Proxmox, máy chủ sẽ bị nghẽn khi có nhiều người cùng tải. Bằng cách offload sang CPU máy khách (Client Compute), máy chủ Proxmox giữ được mức sử dụng CPU tối thiểu.

## 3. Các Tính Năng Trọng Tâm
- **Chế độ Zero-Disk Streaming**: Máy chủ nhận luồng âm thanh từ YouTube CDN và pipe thẳng tới kết nối HTTP của trình duyệt client qua `ReadableStream`, không cần ghi tạm tệp ra đĩa SSD Proxmox.
