...
Đóng gói TuneFlow thành Docker Container tiêu chuẩn, tự động hóa quy trình build và phân phối qua GitHub Actions tới GitHub Container Registry (`ghcr.io/tamld/tuneflow`), đảm bảo người dùng có thể kéo hình ảnh về và chạy ngay trên máy chủ gia đình Proxmox VE (Node 1) chỉ với một lệnh `docker compose up -d`.
...
- **Người dùng gia đình (Ba Mẹ)**: Truy cập dịch vụ ổn định 24/7 qua tên miền nội bộ `nhac.lan`.
...
- **Proxmox Production Compose**: Cung cấp tệp `docker-compose.prod.yml` định nghĩa sẵn tài nguyên giới hạn và nhãn Traefik.
...
- *Ngoài phạm vi*: Xây dựng cluster Kubernetes (đã thống nhất Proxmox chỉ dùng LXC/Docker đơn giản).
